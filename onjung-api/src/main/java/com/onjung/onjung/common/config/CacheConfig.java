...
public class CacheConfig extends CachingConfigurerSupport {
    ...
    public EhCacheCacheManager ehCacheCacheManager(){
        ...
        // 아이템 캐시 설정
        CacheConfiguration itemConfiguration = new CacheConfiguration()
                .eternal(false) //true일 경우 timeout 관련 설정이 무시, element가 캐시에서 삭제되지 않음.
                .timeToIdleSeconds(0) //Element가 지정한 시간 동안 사용(조회)되지 않으면 캐시에서 제거된다. 이 값이 0인 경우 조회 관련 만료 시간을 지정하지 않는다.
                .timeToLiveSeconds(21600) //Element가 존재하는 시간. 이 시간이 지나면 캐시에서 제거된다. 이 시간이 0이면 만료 시간을 지정하지 않는다.
                .maxEntriesLocalHeap(0) //Heap 캐시 메모리 pool size 설정, GC대상이 됨.
        ...
        // 설정을 가지고 캐시 생성
        Cache clientFeedCaching = new Cache(clientFeedConfiguration);
        Cache severFeedCaching = new Cache(severFeedConfiguration);
        Cache itemCaching = new Cache(itemConfiguration);
        ...
        Objects.requireNonNull(cacheManagerFactoryBean().getObject()).addCache(itemCaching);
